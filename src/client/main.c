...
static void CL_TimeBufferStats(int delay, int hi_clamp, int lo_clamp)
{
    if (SCR_XerpDebugLevel() < 2 || cls.demo.playback)
        return;

    if (!xb_stats.frames || delay < xb_stats.delay_min)
        xb_stats.delay_min = delay;
    if (!xb_stats.frames || delay > xb_stats.delay_max)
        xb_stats.delay_max = delay;
    xb_stats.delay_sum += delay;
    xb_stats.frames++;
    xb_stats.hi_clamps += hi_clamp;
    xb_stats.lo_clamps += lo_clamp;
...
static void CL_SetClientTime(void)
{
    ...
    if (cl.time > cl.servertime) {
        ...
        cl.time = cl.servertime;
        cl.lerpfrac = 1.0f;
        CL_TimeBufferStats(0, 1, 0);
    } else if (cl.time < prevtime) {
        SHOWCLAMP(2, "low clamp %i\n", prevtime - cl.time);
        cl.time = prevtime;
        cl.lerpfrac = 0;
        CL_TimeBufferStats(CL_FRAMETIME, 0, 1);
    } else {
        cl.lerpfrac = (cl.time - prevtime) * CL_1_FRAMETIME;
        CL_TimeBufferStats(cl.servertime - cl.time, 0, 0);
    }
