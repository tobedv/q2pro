...
cl_xerp_debug::
    Toggles the netcode instrumentation overlay, drawn at the right edge
    of the screen: current ping, interpolation delay and lerp fraction,
    interval between the two most recent server snapshots, smoothed
    snapshot jitter, the worst snapshot gap observed, and a count of
    stalls (snapshots later than 1.5x the expected interval). Peaks
    reset on level change or by toggling the cvar. Value 2 additionally
    logs a "xerpbuf" timing summary to the console every 5 seconds
    (render-delay distribution, clamp counts, starvation events) for
    validating cl_xerp_buffer settings. Purely diagnostic; hidden
    during demo playback. Default value is 0.

cl_xerp_fire::
    Predicted local weapon fire feedback: your own muzzle flash and fire
    sound play the instant the attack input is sampled instead of after
    the server round-trip; the echoed server muzzle flash is consumed so
    nothing plays twice. Fire effects only — hit detection and damage
    remain fully server-authoritative. Predicts MK23, akimbo, MP5, M4,
    M3, handcannon and the SSG (fire only, incl. zoomed shots; 700 ms
    hold after zoom changes); knife, grenades and silenced shots keep
    standard server-driven behavior. Value 2 additionally logs every fire decision and
    per-shot click-to-echo latency to the console (persist with
    "logfile 2"). Inactive during demo playback and spectating.
    Default value is 0.

cl_xerp_zoom::
    Experimental, keep 0: zoom-in delay is dominated by a deliberate
    ~600 ms server-side weapon settle, not the network, and predicting
    the fov desyncs it from the server-driven scope overlay. Retained
    for experimentation at very high ping. "xerpzoom" telemetry logs
    outcomes. Default value is 0 (recommended).

cl_xerp_ents::
    Client-side extrapolation of remote players (grenades and knives
    stay server-timed — exact position beats freshness): shifts the
    interpolation window one server frame forward (newest snapshot to
    its velocity projection), the same visual effect server-side xerp
    produces, computed locally. Purely visual. Skipped below 120 ups,
    above 2000 ups, and on teleport events. While enabled the client
    reports cl_xerp 0 via CvarSync so use_xerp servers don't double
    extrapolate. cl_xerp_debug 2 logs a "xerpents" validation line
    (extrapolated/fallback counts, prediction error avg/max) every 5
    seconds. Inactive during demo playback. Default value is 0.

cl_xerp_buffer::
    Removed. An adaptive interpolation buffer was invalidated by field
    telemetry: the two-snapshot window cannot hold a positive margin
    (low-clamped away every arrival as 10 Hz time-skips) and the stock
    clamps already ride the minimal interpolation delay. The "xerpbuf"
    render-clock telemetry under cl_xerp_debug 2 remains.
