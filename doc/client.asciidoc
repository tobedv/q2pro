...
cl_xerp_fire::
    ...
    M3, handcannon and the SSG (except for 700 ms after a zoom change);
    knife, grenades and silenced shots keep standard behavior. Value 2 additionally logs every fire decision and
    per-shot click-to-echo latency to the console (persist with
    "logfile 2"). Inactive during demo playback and spectating.
    Default value is 0.

cl_xerp_zoom::
    Predicted sniper zoom: the client mirrors the zoom state machine
    and applies the fov locally the moment the weapon/lens command is
    sent, handing off to the server value on arrival (reverting after
    1.5 s if refused). Purely visual. cl_xerp_debug 2 logs "xerpzoom"
    lines. Inactive during demo playback. Default value is 0.

cl_xerp_ents::
    Client-side extrapolation of remote players, thrown grenades and
    throwing knives (ballistic projection with gravity): shifts the
