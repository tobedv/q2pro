...
cl_xerp_ents::
    Client-side extrapolation of remote players: shifts the
...
cl_xerp_buffer::
    Adaptive interpolation buffer: slews client time toward riding
    [value + 3x measured snapshot jitter] ms behind the newest snapshot
    instead of the connect-timing-random 0-100 ms, giving a fresher view
    and faster kill feedback. Slew is capped at 2 ms per render frame;
    starvation raises the margin for a few seconds. Watch "interp" in
    the cl_xerp_debug overlay while tuning. Inactive during demo
    playback. Default value is 0 (disabled).
