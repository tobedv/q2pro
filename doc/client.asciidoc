...
cl_xerp_buffer::
    Experimental, keep 0: telemetry invalidated the premise. The
    two-snapshot interpolation window cannot hold a positive margin —
    it is low-clamped away at every snapshot arrival, producing 10 Hz
    time-skips — and the engine's clamps already self-stabilize at the
    minimum interpolation delay. Retained for experimentation only.
    Default value is 0 (disabled — recommended).
